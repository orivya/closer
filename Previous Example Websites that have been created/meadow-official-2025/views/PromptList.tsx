...
const PromptList: React.FC<PromptListProps> = ({ onChangeView, categoryId }) => {
  ...
  const prompts = category ? category.prompts : PROMPT_CATEGORIES.flatMap(c => c.prompts.slice(0, 6));
  ...
  return (
    <div className="animate-fade-up max-w-4xl mx-auto space-y-8 pb-20">
       <div className="text-center py-8">
          ...
          <p className="text-xs text-text-muted mt-4">{promptCount} prompts available</p>
       </div>

       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {prompts.map((prompt, idx) => (
             <div key={idx} className="glass-card p-8 rounded-[24px] hover:shadow-glow hover:border-sage-border transition-all flex flex-col justify-between group min-h-[200px]">
                <p className="font-serif text-lg text-text-primary leading-relaxed mb-6 group-hover:text-sage transition-colors break-words">
                   "{prompt}"
                </p>
       ...
       <div className="text-center pt-8">
          <button className="inline-flex items-center gap-2 px-6 py-3 bg-dark-surface border border-dark-border rounded-full text-sm font-medium text-text-secondary hover:bg-dark-hover hover:border-sage-border transition-all">
