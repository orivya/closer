      ...
      <div className="max-w-[1200px] mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-12">
          {GENRES.map((genre) => (
            <button
              ...
              className={`px-6 py-2 text-sm font-semibold rounded-full border transition-all ${
                activeGenre === genre
        ...
        <div className="max-w-3xl mx-auto bg-[var(--bg-elevated)] border border-[var(--border-dark)] rounded-3xl p-8 shadow-2xl">
          <div className="flex items-center gap-4 mb-8">
            ...
            >
              <span className={`text-xs font-bold uppercase tracking-wider ${isMixed ? 'text-white' : 'text-[var(--text-gray)]'}`}>
                {isMixed ? 'Mixed' : 'Raw'}
              </span>
              <div className={`w-10 h-5 rounded-full relative transition-colors ${isMixed ? 'bg-[var(--accent)]' : 'bg-[var(--bg-card)] border border-[var(--text-muted)]'}`}>
                <div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${isMixed ? 'translate-x-5' : 'translate-x-0'}`} />
              </div>
            </button>
          </div>
          ...
          <div className="relative h-24 bg-[var(--bg-card)] rounded-xl mb-8 overflow-hidden group cursor-pointer" onClick={() => setIsPlaying(!isPlaying)}>
            <div className="absolute inset-0 flex items-center justify-center gap-1 px-4">
              {[...Array(40)].map((_, i) => {
                const height = isPlaying ? Math.random() * 80 + 20 : [50, 35, 70, 90, 45, 100, 60, 40, 80, 55][i % 10];
                return (
                  <div 
                    key={i} 
