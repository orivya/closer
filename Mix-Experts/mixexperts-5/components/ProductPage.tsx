...
import { Check, Star, Download, Play, Monitor, Cpu, ChevronDown } from 'lucide-react';
...
export const ProductPage: React.FC<ProductPageProps> = ({ onBack, product }) => {
  return (
    <div className="pt-24 min-h-screen bg-[var(--bg-base)] animate-in fade-in duration-500">
      
      {/* Dynamic SEO Tags */}
      <Helmet>
        <title>{product.title} | James Mix Audio</title>
        <meta name="description" content={`Get the ${product.title}. Professional ${product.type} for logic pro and other DAWs. Instant download.`} />
        <meta property="og:title" content={product.title} />
        <meta property="og:description" content={`Get the ${product.title}. Professional ${product.type}.`} />
        <meta property="og:image" content={product.image} />
        <meta property="og:type" content="product" />
        <meta property="og:price:amount" content={product.price.replace('$', '')} />
