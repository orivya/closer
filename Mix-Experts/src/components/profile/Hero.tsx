...
export const Hero: React.FC<HeroProps> = ({ profile }) => {
    ...
    const tagline = profile.tagline || 'Professional Audio Engineer';
    const bio = profile.bio || 'Professional audio engineering services tailored to your needs.';
    const avatarUrl = profile.avatar_url || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400&h=400&fit=crop&crop=faces';

    // Split tagline into parts for styling (if it contains a period)
    const taglineParts = tagline.includes('.')
        ? tagline.split('.').filter(Boolean).slice(0, 2)
        : [tagline];
                        ...
                        <h1 className="text-6xl md:text-8xl lg:text-9xl font-bold tracking-tighter text-white mb-8 leading-[0.9]">
                            {taglineParts.length > 1 ? (
                                <>
                                    ...
                                    <br />
                                    <span className="text-[var(--text-gray)]">{taglineParts[1]}</span>
                                    <span className="text-[var(--accent)]">.</span>
                                </>
                            ) : (
                                <>
